...
phase["throat.diffusive_conductance"][
    n_al + n_lic6 + n_sep : n_al + n_lic6 + n_sep + n_lico
] = a_lico
phase["throat.diffusive_conductance"][
    n_al + n_lic6 + n_sep + n_lico : n_al + n_lic6 + n_sep + n_lico + n_cu
] = a_cu
...
alg.run()
deff_series = alg.calc_effective_diffusivity(
    domain_area=1.0, domain_length=np.sum(lens)
)[0]
...
alg.run()
deff_parallel = alg.calc_effective_diffusivity(
    domain_area=np.sum(lens), domain_length=1.0
)[0]
print(
    "Series",
    deff_series,
