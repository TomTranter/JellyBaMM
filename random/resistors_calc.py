...
phase['throat.diffusive_conductance'][n_al + n_lic6 + n_sep:n_al +
                                      n_lic6 + n_sep + n_lico] = a_lico
phase['throat.diffusive_conductance'][n_al + n_lic6 + n_sep + n_lico:n_al +
                                      n_lic6 + n_sep + n_lico + n_cu] = a_cu

diffs = phase['throat.diffusive_conductance'].copy()
alg = op.algorithms.FickianDiffusion(network=net_series)
alg.setup(phase=phase)
alg.set_value_BC(pores=net_series.pores('left'), values=1.0)
alg.set_value_BC(pores=net_series.pores('right'), values=0.0)
alg.run()
deff_series = alg.calc_effective_diffusivity(domain_area=1.0,
                                             domain_length=np.sum(lens))[0]
net_parallel = op.network.Cubic([2, np.sum(lens), 1])
Ts = net_parallel.find_neighbor_throats(pores=net_parallel.pores('left'), mode='xor')
...
alg.setup(phase=phase)
alg.set_value_BC(pores=net_parallel.pores('left'), values=1.0)
alg.set_value_BC(pores=net_parallel.pores('right'), values=0.0)
alg.run()
deff_parallel = alg.calc_effective_diffusivity(domain_area=np.sum(lens),
                                               domain_length=1.0)[0]
print('Series', deff_series, 'Parallel', deff_parallel,
      'Ratio', deff_parallel / deff_series)
