...
exp_files = ["MJ1_0.5C.csv", "MJ1_1.0C.csv", "MJ1_1.5C.csv"]
            ...
            for vi, v in enumerate(variables):
                ...
                if temp is not None:
                    ...
                    for t in range(temp.shape[0]):
                        (mean, std_dev) = jellybamm.weighted_avg_and_std(
                            temp[t, :], weights
                        )
                        means[t] = mean
...
fig1 = jellybamm.jellyroll_subplot(
    d, 2, amps[-1], var=0, soc_list=soc_list, global_range=False, dp=1
)
if savefigs:
...
fig4 = jellybamm.jellyroll_subplot(
    d, 7, amps[-1], var=0, soc_list=soc_list, global_range=False, dp=1
)
if savefigs:
    plt.savefig(os.path.join(save_im_path, "fig4.png"), dpi=600)
# 3rd Case 5.25 Amps - HTC 100 - 5 Tab
fig5 = jellybamm.jellyroll_subplot(
    d, 12, amps[-1], var=0, soc_list=soc_list, global_range=False, dp=1
)
if savefigs:
...
# Third Heating
fig12 = jellybamm.jellyroll_subplot(
    d, 19, 5.25, var=0, soc_list=soc_list, global_range=False
)
if savefigs:
    plt.savefig(os.path.join(save_im_path, "fig12.png"), dpi=600)
fig13 = jellyroll_one_plot(
    d[19][5.25][1]["data"][-1, :],
...
figY = jellybamm.jellyroll_subplot(
    d, 19, 5.25, var=1, soc_list=[[0.9, 0.7], [0.5, 0.3]], global_range=False, dp=1
)
if savefigs:
