...
for key in cases.keys():
    ...
    for amp in amps:
        ...
        for i in range(data.shape[1]):
            (dist_name, chi_square, dist, arg, dist_mean, dist_std) = find_best_fit(
                data[:, i]
            )
        ...
        io.savemat(
            file_name=save_file + "_std", mdict={"data": stds}, long_field_names=True
        )
        if savefigs:
            plt.savefig(os.path.join(save_file + "_log_std.png"), dpi=600)
        jellyroll_one_plot(means, "Current Density Distribution Means")
        io.savemat(
            file_name=save_file + "_mean", mdict={"data": means}, long_field_names=True
        )
        if savefigs:
